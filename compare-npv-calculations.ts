...
async function compareNPVCalculations() {
  try {
    ...
    console.log('='.repeat(80));
    console.log('contractRentNPV:', (parseFloat(metrics.contractRentNPV)/1e6).toFixed(2), 'M');
    console.log('contractRentNPV (abs):', (Math.abs(parseFloat(metrics.contractRentNPV))/1e6).toFixed(2), 'M');
    ...
    const contractPeriods = periods.filter((p) => p.year >= startYear && p.year <= endYear);
    console.log('Contract periods (2028-' + endYear + '):', contractPeriods.length);
    ...
    console.log('='.repeat(80));
    console.log('Frontend NPV (abs):', npvFrontend.abs().dividedBy(1e6).toFixed(2), 'M');
    console.log('Backend NPV (abs):', npvBackend.abs().dividedBy(1e6).toFixed(2), 'M');
    console.log('Stored NPV (abs):', (Math.abs(parseFloat(metrics.contractRentNPV))/1e6).toFixed(2), 'M');

    console.log('\n' + '='.repeat(80));
    console.log('DISCREPANCY ANALYSIS');
    console.log('='.repeat(80));
    const diff1 = npvFrontend.abs().minus(new Decimal(Math.abs(parseFloat(metrics.contractRentNPV))));
    const diff2 = npvBackend.abs().minus(new Decimal(Math.abs(parseFloat(metrics.contractRentNPV))));

    console.log('Frontend vs Stored:', diff1.dividedBy(1e6).toFixed(2), 'M difference');
    console.log('Backend vs Stored:', diff2.dividedBy(1e6).toFixed(2), 'M difference');
    ...
    console.log('Difference:', ((storedNPVAbs - expectedNPV)/1e6).toFixed(2), 'M');
    console.log('Percentage difference:', (((storedNPVAbs - expectedNPV)/expectedNPV)*100).toFixed(1), '%');
