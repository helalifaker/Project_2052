...
export function ControlPanel() {
  ...
  const handleDiscountRateChange = useCallback(
    (vals: number[]) => updateAssumption("discountRate", vals[0] / 100),
    [updateAssumption],
  );

  const handleInflationRateChange = useCallback(
    (vals: number[]) => updateAssumption("inflationRate", vals[0] / 100),
    [updateAssumption],
  );

  const handleRentGrowthFactorChange = useCallback(
    (vals: number[]) => updateAssumption("rentGrowthFactor", vals[0] / 100),
    [updateAssumption],
  );
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <Slider
            ...
            max={150}
            step={5}
            onValueChange={handleRentGrowthFactorChange}
            className="w-full"
          />
          <p className="text-xs text-muted-foreground">
