...
const ExecutiveCard = React.forwardRef<HTMLDivElement, ExecutiveCardProps>(
    ({ className, size = 'default', ...props }, ref) => (
        <div
            ref={ref}
            className={cn("executive-card", className)}
            {...props}
        />
    )
);
...
const ExecutiveCardHeader = React.forwardRef<
    HTMLDivElement,
    React.HTMLAttributes<HTMLDivElement> & { size?: 'default' | 'compact' | 'ultra-compact' }
>(({ className, size = 'default', ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
...
const ExecutiveLabel = React.forwardRef<
    HTMLParagraphElement,
    React.HTMLAttributes<HTMLParagraphElement> & { size?: 'default' | 'compact' | 'ultra-compact' }
>(({ className, size = 'default', ...props }, ref) => (
    <p
        ref={ref}
        className={cn(
...
>(({ className, ...props }, ref) => (
    <p
        ref={ref}
        className={cn("executive-subtitle", className)}
        {...props}
    />
));
...
>(({ className, direction = "neutral", size = 'default', ...props }, ref) => (
    <div
        ...
        className={cn(
            ...
            direction === "neutral" && "text-muted-foreground flex items-center gap-2",
            (size === 'compact' || size === 'ultra-compact') ? "text-xs mt-2" : "text-sm mt-4",
            className
        )}
        {...props}
    />
