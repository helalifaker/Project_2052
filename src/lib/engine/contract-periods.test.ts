...
const mockInputBase: CalculationEngineInput = {
  ...
  dynamicPeriodConfig: {
    ...
    staff: {
      fixedStaffCost: new Decimal(12000000),
      variableStaffCostPerStudent: new Decimal(5000),
    },
...
describe.skip("Contract Period Comparison", () => {
  ...
  it("should have higher cumulative metrics for 30-year contract", async () => {
    ...
    expect(
      result30.metrics.totalRent.greaterThan(result25.metrics.totalRent),
    ).toBe(true);
    expect(
      result30.metrics.totalEbitda.greaterThan(result25.metrics.totalEbitda),
    ).toBe(true);
  });
});
