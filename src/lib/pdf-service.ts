...
import { formatCurrency, formatMillions, formatNumber, formatPercent, safeNumber } from "./format-utils";
import { generateExecutiveSummary, generateFinancialAnalysis } from "./report-insights";
...
export async function generateComprehensiveReport(proposal: Proposal, chartImage?: string) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(`Developer: ${proposal.developer || "N/A"}`, margin + 10, detailsY + 8);
    doc.text(`Rent Model: ${proposal.rentModel || "N/A"}`, margin + 10, detailsY + 14);
    doc.text(`Date: ${new Date().toLocaleDateString()}`, pageWidth - margin - 50, detailsY + 8);
    ...
    const kpiData = [
        [
            { content: "Total Rent (30y)", styles: { textColor: [100, 116, 139] as [number, number, number], fontSize: 8 } },
            { content: "NPV", styles: { textColor: [100, 116, 139] as [number, number, number], fontSize: 8 } },
            { content: "IRR", styles: { textColor: [100, 116, 139] as [number, number, number], fontSize: 8 } },
            { content: "Payback", styles: { textColor: [100, 116, 139] as [number, number, number], fontSize: 8 } }
        ],
        [
            { content: formatMillions(metrics?.totalRent), styles: { fontSize: 14, fontStyle: "bold" as const, textColor: [15, 23, 42] as [number, number, number] } },
            { content: formatMillions(metrics?.npv), styles: { fontSize: 14, fontStyle: "bold" as const, textColor: (safeNumber(metrics?.npv) >= 0 ? [21, 128, 61] : [185, 28, 28]) as [number, number, number] } },
            { content: formatPercent(metrics?.irr), styles: { fontSize: 14, fontStyle: "bold" as const, textColor: [15, 23, 42] as [number, number, number] } },
            { content: `${safeNumber(metrics?.paybackPeriod).toFixed(1)} Yrs`, styles: { fontSize: 14, fontStyle: "bold" as const, textColor: [15, 23, 42] as [number, number, number] } }
        ]
    ];
    ...
    const assumptionsData = [
        ["Contract Period", `${safeNumber((proposal as any).contractPeriodYears) || 30} Years`],
        ["Rent Model", proposal.rentModel || "N/A"],
        ["Base Rent / Share", proposal.rentModel === "FIXED" ? formatCurrency((proposal.rentParams as any)?.baseRent) : formatPercent((proposal.rentParams as any)?.revenueSharePercent)],
        ["Escalation", proposal.rentModel === "FIXED" ? `${formatPercent((proposal.rentParams as any)?.growthRate)} every ${(proposal.rentParams as any)?.frequency} years` : "N/A"],
        ["Capacity", `${formatNumber((proposal.enrollment as any)?.totalCapacity)} Students`],
        ["Steady State", `${formatNumber((proposal.enrollment as any)?.steadyStateStudents)} Students`],
        ["IB Program", (proposal.curriculum as any)?.ibProgramEnabled ? "Yes" : "No"],
    ];
    ...
    // Define Pivot Years
    const pivotYears = [2023, 2024, 2025, 2028, 2029, 2030, 2040, 2041, 2052, 2053];
    const financials = proposal.financials || [];

    // Filter data for pivot years
    const pivotData = pivotYears.map(year => {
        const yearData = financials.find((f: any) => f.year === year);
        return {
            year,
            ...
            cashFlow: safeNumber((yearData?.cashFlow as any)?.netChangeInCash)
        };
    });
