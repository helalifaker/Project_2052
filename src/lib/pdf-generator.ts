...
export const generateProposalPDF = async (
  ...
  const addHeader = (pageNumber: number) => {
    ...
    doc.text(`Page ${pageNumber}`, pageWidth - margin - 10, 10);
  ...
  // Pivot Years Logic
  const allYears =
    (proposal.financials as FinancialRecord[] | undefined)?.map(
      (f) => f.year,
    ) || [];
  const pivotYears = [
    2023,
  ...
  const tableData = pivotYears.map((year) => {
    const fin = (proposal.financials as FinancialRecord[] | undefined)?.find(
      (f) => f.year === year,
    );
    if (!fin) return [];
    return [
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- jsPDF autotable extends the type
  ...
  doc.setTextColor(60);
  const commentary =
    "The financial projections indicate a steady ramp-up in revenue starting from 2028. Operational expenses are optimized, ensuring a healthy EBITDA margin throughout the lease term.";
