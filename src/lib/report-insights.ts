...
export function generateExecutiveSummary(proposal: any): string {
...
export function generateFinancialAnalysis(proposal: any): string {
    const metrics = proposal.metrics || {};
    const totalRevenue = new Decimal(metrics.totalRevenue || 0);
    const totalEbitda = new Decimal(metrics.totalEbitda || 0);
    const margin = totalRevenue.isZero() ? 0 : totalEbitda.dividedBy(totalRevenue).toNumber();

    const rentModel = proposal.rentModel;
    let rentCommentary = "";

    if (rentModel === "Fixed Escalation") {
        rentCommentary = "The Fixed Escalation model provides predictable cost structures, allowing for easier long-term budgeting, though it lacks flexibility in downturns.";
    } else if (rentModel === "Revenue Share") {
        rentCommentary = "The Revenue Share model aligns landlord incentives with school performance, reducing risk during the ramp-up phase but potentially increasing costs in high-performance years.";
    } else {
        rentCommentary = "The Partner model balances initial capital expenditure with long-term operational costs.";
    }
