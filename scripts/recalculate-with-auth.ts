...
async function recalculate() {
  try {
    ...
    console.log(`\n📊 System Config:`);
    console.log(
      `  Discount Rate: ${input.systemConfig.discountRate?.toString() ?? "NOT SET"}`,
    );
    console.log(`  WACC: ${input.systemConfig.wacc?.toString() ?? "NOT SET"}`);
    console.log(
      `  Debt Interest Rate: ${input.systemConfig.debtInterestRate.toString()}`,
    );
    ...
    console.log(
      `  Effective NPV Rate: ${effectiveRate.toString()} (${effectiveRate.times(100).toString()}%)\n`,
    );
    ...
    console.log(
      `  Contract Total Rent: ${result.metrics.contractTotalRent.toString()}`,
    );
    console.log(
      `  Contract Rent NPV: ${result.metrics.contractRentNPV.toString()}`,
    );
    console.log(
      `  Contract Total EBITDA: ${result.metrics.contractTotalEbitda.toString()}`,
    );
    ...
    console.log(
      `  ${result.metrics.contractRentNPV.abs().dividedBy(1_000_000).toDecimalPlaces(1).toString()}M`,
    );
  } catch (error) {
