  ...
  try {
    ...
    if (proposal.financials) {
      ...
      if (financials.contractPeriod) {
        ...
        console.log(
          "Total Rent (Contract):",
          financials.contractPeriod.totalRent,
        );
        console.log("NPV (Contract):", financials.contractPeriod.npv);
        console.log(
          "Total EBITDA (Contract):",
          financials.contractPeriod.totalEbitda,
        );
        console.log(
          "Contract End Year:",
          financials.contractPeriod.contractEndYear,
        );
      }
      ...
      if (periods && periods.length > 0) {
        ...
        periods.forEach((period: any) => {
          ...
          if (year <= 2028) {
            console.log(
              `Year ${year}: Rent=${rent.toFixed(0)}, PV=${presentValue.toFixed(0)}`,
            );
          }
        });

        console.log(
          "\nManual NPV (10% discount):",
          (manualNPV / 1_000_000).toFixed(1),
          "M",
        );
      }
    }
