...
async function diagnoseRentNPV() {
  try {
    ...
    rentCashFlows.forEach(({ year, rent, pv }) => {
      console.log(
        `Year ${year}: Rent = ${(rent / 1e6).toFixed(2)}M, PV = ${(pv / 1e6).toFixed(2)}M`,
      );
    });
    ...
    console.log("=".repeat(80));
    console.log(
      "Manual NPV (negative, tenant perspective):",
      (manualNPV / 1e6).toFixed(2),
      "M",
    );
    console.log(
      "Manual NPV (positive, landlord perspective):",
      (manualNPVAbs / 1e6).toFixed(2),
      "M",
    );
    console.log(
      "Stored contractRentNPV:",
      (parseFloat(metrics.contractRentNPV) / 1e6).toFixed(2),
      "M",
    );
    console.log(
      "Difference:",
      ((manualNPV - parseFloat(metrics.contractRentNPV)) / 1e6).toFixed(2),
      "M",
    );
    ...
    console.log("=".repeat(80));
    console.log(
      "1. Absolute value (landlord perspective):",
      Math.abs(manualNPV / 1e6).toFixed(2),
      "M",
    );
    console.log(
      "2. EBITDA NPV (value created):",
      (parseFloat(metrics.contractEbitdaNPV) / 1e6).toFixed(2),
      "M",
    );
    console.log(
      "3. Net Tenant Surplus (EBITDA - Rent):",
      (parseFloat(metrics.contractNetTenantSurplus) / 1e6).toFixed(2),
      "M",
    );
    ...
    for (const testRate of [0.05, 0.06, 0.07, 0.08, 0.1, 0.12]) {
      let testNPV = 0;
      periods.forEach((period: any) => {
        const year = period.year;
        if (year >= contractStartYear && year <= contractEndYear) {
      ...
      console.log(
        `Rate ${(testRate * 100).toFixed(0)}%: ${(testNPV / 1e6).toFixed(1)}M`,
      );
